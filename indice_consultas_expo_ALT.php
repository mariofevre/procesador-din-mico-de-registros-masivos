...
function consultasCargadas(response){	
	...
	if(_res.res=='exito'){
		...
		for(_idCon in _res.data){
			...
			_tab=document.createElement("table");
			_tab.innerHTML="<tr><th>categoria</th><th>cant.</th><th>% enc</th><th>% inc</th></tr>";
			_dat.appendChild(_tab);
			...
			for (_idGrupo in _res.data[_idCon].grupos) {
				...
				_tDc = document.createElement("td");
				_tr.appendChild(_tDc);
				_tDc.innerHTML=_res.data[_idCon].grupos[_idGrupo].cant;
				...
				if(_res.data[_idCon].tipo=='Agrupación'){		
					for(_idcaso in _res.data[_idCon].grupos[_idGrupo].casos){
						...
						_parr = document.createElement("p");
						_td.appendChild(_parr);
						...
						if(_estecaso.CampoDesc===null){
							...
							if(_estecaso.especial=='otros'){
								_parr.innerHTML+= "Otros valores";
							}else{
								_parr.innerHTML+=_estecaso.CampoDesc.Comment.substring(0,10);
								if((_estecaso.valor != null)){
									_parr.innerHTML+= " = " + _estecaso.valor;
								}else if((_estecaso.valorno != null)){
									_parr.innerHTML+= " != " + _estecaso.valorno;
								}else{
									if((_estecaso.valmin != null)){
										_parr.innerHTML+= " >= " + _estecaso.valmin;
									}
									if((_estecaso.valmax != 'null')){
										_parr.innerHTML+= " <= " + _estecaso.valmax;
									}
							}
							}
						}
