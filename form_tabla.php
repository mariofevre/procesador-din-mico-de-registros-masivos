...
//if($_SERVER[SERVER_ADDR]=='192.168.0.252')ini_set('display_errors', '1');ini_set('display_startup_errors', '1');ini_set('suhosin.disable.display_errors','0'); error_reporting(-1);
	...
	$campofijoc = $_GET["campofijoc"];
	if($campofijoc == ""){$campofijoc = "vacio";}
	$campofijoc_c = $_GET["campofijoc_c"];

	$Salida = $_GET["salida"];	
	if($Salida==''){$Salida='indice_contenidos';}
	
	$Salidaid = $_GET["salidaid"];
	if($Salidaid==''){$Salidaid=$_GET['id_p_PROproyectos'];}
	if($Salidaid==''){$Salidaid=$_GET['id'];}
		
	$Salidatabla = $_GET["salidatabla"];
	
	//$ConUsu = mysql_query("usuarios", $query,$Conec1);	
	
	mysql_select_db ('TReCCsound',$Conec1);
	...
	?>
			<div id="marco">
				<form action="./ed_<?php echo $Href;?>" method="POST" enctype='multipart/form-data'>
					...
					<input type="hidden" name="<?php echo $campofijoc;?>" value="<?php echo $campofijoc_c;?>">							
					...
					<div id="hoja">	
					    ...
					    if (mysql_num_rows($result) > 0) {					    	
					    	 if(mysql_result($Consulta, 0, 'id_p_PROproyectos')>0){
					    	 	$Idproy=mysql_result($Consulta, 0, 'id_p_PROproyectos');
								 echo "..$Idproy ..";
					    	 }
							$empaquetado = -100;
							$borradato="no";
					        while ($row = mysql_fetch_assoc($result)){
								...
								if($row['Field'] == 'id'){
								...
								}elseif($row['Field'] == $campofijoc){
									echo "<p>".$comentarioc.": ".$campofijoc_c."</p>";
									...
									echo "<a onclick='this.previousSibling.previousSibling.value=\"-[-BORRX-]-\"'>[x]</a>";
										...
										if($Typolink == "id_p"){	
											...
											/* para tablas padre en otras bases*/
						  						...
						  						if (mysql_num_rows($estructurapadre) > 0) {
													...
													$wheremas = "WHERE '1'='1'";
						  							while ($value = mysql_fetch_assoc($estructurapadre)){
														...
														if($value['Field']=='id_p_PROproyectos'&&isset($Idproy)){
															$wheremas .= " AND id_p_PROproyectos='".$Idproy."'";
														}
													}		
												...
												if (mysql_num_rows($result) > 0) {
													...
													$readonly = "";/* algunos campos no permiten la autogeneración de nuevos registros asociados*/
					...
					<input type="button" value="Cancelar" onclick="window.location.href='./<?php echo $Salida;?>.php?salida=<?php echo $Salida;?>&id=<?php echo $Salidaid;?>&tabla=<?php echo $Salidatabla;?>';">
					...
					if($Campo==''&&$Accion!='agrega'){
						...
						?>
						<input type="button" value="Recuperar" onclick="window.location.href = './form_tabla.php?tabla=<?php echo $Tabla;?>&id=<?php echo $Id;?>&accion=recupera&salida=<?php echo $Salida;?>&salidaid=<?php echo $Salidaid;?>&salidatabla=<?php echo $Salidatabla;?>';">
						<?php	
						}else{
						?>
						<input type="button" value="Eliminar" onclick="window.location.href = './form_tabla.php?tabla=<?php echo $Tabla;?>&id=<?php echo $Id;?>&accion=borra&salida=<?php echo $Salida;?>&salidaid=<?php echo $Salidaid;?>&salidatabla=<?php echo $Salidatabla;?>';">
						<?php
						}
					}
